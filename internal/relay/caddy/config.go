...
// fields are deliberately generic so the data model carries the extensibility
// seams from the design spec: Role distinguishes "server" from "relay" (the
// relay's own admin route), and Upstream is a field rather than a hardcoded
// SSH-port assumption.
type Server struct {
